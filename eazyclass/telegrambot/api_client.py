...
class ApiClient:
    ...
    #     reraise=True
    # )
    async def _send_request(self, method: str, url: str, body_json: bytes, headers: dict) -> Dict[str, Any]:
        """Отправка запроса с повторными попытками."""
        async with self.session.request(method, url, data=body_json, headers=headers) as response:
    ...
    async def request(self, social_id, endpoint: str, method: str = "POST", payload: Dict[str, Any] = None) -> Dict[str, Any]:
        """Выполняет запрос к Django API с HMAC-подписью."""
        if not self.session or self.session.closed:
            await self.start()

        url = urljoin(self.base_url, endpoint)
        body_json = json.dumps(payload, ensure_ascii=False).encode("utf-8") if payload else b""
        headers = self._generate_hmac_headers(method, url, body_json, social_id)

        try:
            return await self._send_request(method, url, body_json, headers)
        except (aiohttp.ClientError, asyncio.TimeoutError) as e:
            logger.error(f"Request failed after retries: {str(e)}")
            raise

    async def bot_request(self, endpoint: str, method: str = "POST", payload: Dict[str, Any] = None) -> Dict[str, Any]:
        return await self.request(self.bot_social_id, endpoint, method, payload)
