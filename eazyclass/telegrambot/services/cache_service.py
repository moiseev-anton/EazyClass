...
class CacheService:
    ...
    async def update_data(self, endpoint: str, file_path: str) -> dict:
        ...
        try:
            ...
            if not isinstance(data, dict):
                logger.warning(
                    f"Invalid data format from API ({endpoint}), falling back to file."
                )
        ...
        try:
            ...
            logger.error(
                f"Failed to load keyboard data from file ({file_path}): {str(e)}"
            )
            return {}

    async def update_faculties(self) -> dict[str, Any]:
        """Обновляет кэш факультетов и сохраняет в указанный файл."""
        faculties_data = await self.update_data(
            "bot-faculties/", self.faculties_cache_file
        )
        self.cache_manager.faculties = faculties_data
    ...
    async def update_teachers(self) -> dict[str, Any]:
        ...
        teachers_data = await self.update_data(
            "bot-teachers/", self.teachers_cache_file
        )
        self.cache_manager.teachers = teachers_data
