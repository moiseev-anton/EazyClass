...
async def test_relationships_with_context_manager_async_async(mocked_fetch, api_schema):
    async with Session('http://localhost:8080/api', schema=api_schema) as s:
        ...
        assert ext_ref_res.id == 'qvantelLease1Extref'
        ...
        assert d1.user_account.resource.id == 'qvantelUseraccount1'
        assert d1.user_account.resource.type == 'userAccounts'
        assert d1.links.self.href == '/api/leases/qvantelLease1'
...
async def test_more_relationships_async_fetch(mocked_fetch, api_schema):
    async with Session('http://localhost:8080/api', schema=api_schema) as s:
        ...
        # fetches http://localhost:8080/api/leases/qvantelLease1/externalReferences
        ...
        assert ext_ref.id == 'qvantelLease1Extref'
        ...
        # Fetches http://localhost:8080/api/leases/qvantelLease1/parentLease
        parent_lease = d1.parent_lease
        assert isinstance(parent_lease, jsonapi_client.relationships.LinkRelationship)
...
async def test_patching(mocker, mocked_fetch, api_schema, mock_update_resource):
    mock_patch = mocker.patch('aiohttp.ClientSession.request')
    ...
    with pytest.raises(AttributeError):
        resources.someattribute = 'something'

    r1 = resources[0]

    # Let's change fields in resourceobject
    assert r1.active_status == 'active'
    r1.active_status = 'terminated'
    assert r1.is_dirty
    assert s.is_dirty
    assert 'activeStatus' in r1.dirty_fields
    await r1.commit()  # alternatively s.commit() which does commit for all dirty objects
    assert len(r1.dirty_fields) == 0
    assert not r1.is_dirty
    assert not s.is_dirty

    assert r1.valid_for.start_datetime == "2015-07-06T12:23:26.000Z"

    r1.valid_for.start_datetime = 'something-else'
    r1.valid_for.new_field = 'something-new'
    assert r1.valid_for.is_dirty
    assert 'startDatetime' in r1.valid_for._dirty_attributes
    assert r1.is_dirty
    assert 'validFor' in r1.dirty_fields

    assert r1._attributes.diff == {'validFor': {'startDatetime': 'something-else',
                                                       'newField': 'something-new'}}

    ext_refs: list = await r1.external_references.fetch()
    assert r1.external_references.resources[0].id == 'qvantelLease1Extref'
    assert len(r1.external_references.resources) == 1

    ext_refs += [ResourceTuple(str(i), 'externalReferences') for i in [1, 2]]
    r1.external_references.set([r.id for r in ext_refs], 'externalReferences')

    assert len(r1.relationships.external_references.document.resources) == 1
    assert len(r1.external_references.resources) == 3

    ext_refs += [ResourceTuple('3', 'external-references')]
    r1.external_references.set([r.id for r in ext_refs], 'externalReferences')
    assert len(r1.relationships.external_references.document.resources) == 1
    assert len(ext_refs) == 4

    assert r1.relationships.external_references.is_dirty

    assert len(mock_patch.mock_calls) == 1
    await r1.commit()
    assert len(mock_patch.mock_calls) == 2
    args, kwargs = mock_patch.call_args_list[1]
    data_str = kwargs['data']
    actual_data = json.loads(data_str)['data']
    expected_data = {
        'id': 'qvantelLease1',
        'type': 'leases',
        'attributes': {
    ...
    assert not r1.is_dirty
    assert not r1.valid_for.is_dirty
    assert not r1.relationships.external_references.is_dirty
...
async def test_result_pagination(mocked_fetch, api_schema):
    ...
    agr_next = await doc.links.next.fetch()
    while agr_next:
    ...
    d1 = doc.resources[0]
    ext_refs = await d1.external_references.fetch()
...
async def test_result_filtering(mocked_fetch, api_schema):
    async with Session('http://localhost:8080/', schema=api_schema) as s:
        ...
        result = await ext_refs.filter(Filter(title='Hep'))
...
article_test_schema = \
    {
        'articles': {
            'properties': {
                'title': {'type': 'string'},
                'extraAttribute': {'type': ['string', 'null']},
...
async def test_posting_successfull_with_predefined_fields(kw_format, commit, mock_req_async, mocker):
    ...
    kwargs2 = dict(valid_for={'startDatetime': 'asdf'})
    ...
    agr_data = {
        'data': {
            ...
            'relationships': {
                'leaseItems': {
                    'data': [{'id': '1', 'type': 'leaseItems'}]
                },
            }
        }
    }

    mock_req_async.assert_called_once_with('post', 'http://localhost:80801/api/leases', agr_data)
    await s.close()
...
async def test_create_with_underscore(mock_req_async):
    ...
    a = s.create('articles',
                 fields={'withCamel': 'test', 'with_underscore': 'test2'}
    )
    assert 'with_underscore' in a._attributes
    assert 'withCamel' in a._attributes
...
async def test_create_with_underscore2(mock_req_async):
    s = Session('http://localhost:8080/', schema=underscore_schema)
    a = s.create('articles', with_camel='test',
...
async def test_relationship_manipulation_alternative_api(mock_req_async, mocked_fetch, article_schema, mock_update_resource):
    async with Session('http://localhost:80801/', schema=article_schema) as s:
        article = (await s.get('articles')).resource
        ...
        # this.
        comments = await article.comments.fetch()
        oc1, oc2 = comments
...
@pytest.mark.asyncio
async def test_posting_async_with_custom_header(event_loop, session):
    ...
    response.status = 500
    response.get_encoding = mock.Mock(return_value='utf-8')
...
@pytest.mark.asyncio
async def test_error_handling_async_get(event_loop, session):
...
@pytest.mark.asyncio
async def test_error_handling_posting_async(event_loop, session):
